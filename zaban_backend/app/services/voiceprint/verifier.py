...
class VoiceVerifierECAPA:
    ...
    def extract_embedding(self, audio_path: Union[str, np.ndarray, dict]) -> np.ndarray:
        ...
        return self._embedder.extract_embedding(audio, sample_rate=settings.TARGET_SAMPLE_RATE)
    ...
    def enroll_user(
        ...
        embs = [self.extract_embedding(p) for p in audio_paths]
    ...
    def verify_speaker(
        ...
        # Compute raw PLDA score
        raw_score = plda_score(user_emb, test_emb, self._plda)
        
        # Get cohort vectors for AS-Norm
        cohort_enroll = get_top_k_cohort_vectors(
            self.qdrant_client, user_emb, self.cohort_top_k
        )
        cohort_test = get_top_k_cohort_vectors(
            self.qdrant_client, test_emb, self.cohort_top_k
        )
        ...
        # Compute cohort PLDA scores
        scores_enroll = compute_cohort_plda_scores(user_emb, cohort_enroll, self._plda)
        scores_test = compute_cohort_plda_scores(test_emb, cohort_test, self._plda)
    ...
    def list_enrolled_users(self, limit: int = 100) -> List[Dict]:
        """
