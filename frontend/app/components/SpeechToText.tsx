...
export default function SpeechToText({ onTranscriptionComplete }: SpeechToTextProps) {
  ...
  const processAudio = async () => {
    ...
    try {
      ...
      const result = await transcribeAudio(audioFile, language || undefined, model);
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-800 mt-1">Record or upload audio to transcribe</p>
        </div>
      </div>
      ...
      <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        <div className="space-y-3">
          <label className="flex items-start gap-3 p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
            ...
            <div className="flex-1">
              <span className="text-sm font-medium text-gray-900 block">Whisper</span>
              <span className="text-xs text-gray-700 mt-1 block">Auto-detects language, supports 100+ languages</span>
            </div>
          </label>
          <label className="flex items-start gap-3 p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
            <input
            ...
            />
            <div className="flex-1">
              <span className="text-sm font-medium text-gray-900 block">AI4Bharat</span>
              <span className="text-xs text-gray-700 mt-1 block">Best for Indian languages, requires language selection</span>
            </div>
          </label>
        </div>
        ...
        {model === 'ai4bharat' && (
          <div className="mt-4">
            ...
            <p className="text-xs text-gray-700 mt-2">
              AI4Bharat model works best for Indian languages: hi, mr, ta, te, bn, gu, kn, ml, pa, or, as, ne, en
            </p>
          </div>
        )}
        ...
        {model === 'whisper' && (
          <div className="mt-4">
            ...
            <p className="text-xs text-gray-700 mt-2">
              Whisper can auto-detect the language. You can specify it manually for better accuracy.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
            ...
            >
              {isRecording ? (
                ...
                <>
                  ...
                  <span className="text-xs text-gray-700 mt-1">Click to start</span>
                </>
              )}
            </motion.button>
        ...
        {transcription && (
          ...
          >
            ...
            <p className="text-sm text-gray-900 leading-relaxed">{transcription}</p>
          </motion.div>
        )}
      </div>
